...
def bezier_interpolation(points, points_to_compute, w):
      ...
      - `w`: A list of weights for control points.
      '''

      control_points = numpy.array(points)
      ...
      for t in ts:
         ...
         num = numpy.column_stack((coefs*w*numpy.column_stack(control_points))).sum(axis=0)
         den = (coefs*w).sum(axis=0)
         if den == 0:
            den = 1
         point = tuple(num/den)
         result.append(point)
